...
void Tree::insert(const string &word) {
    ...
    if(root->small == ""){
        ...
        while(cur) {
            if(cur->left == 0 && cur->middle == 0 && cur->right == 0) {      //restore branch   //make sure it is leaf
                if(cur->large == "") {      //large is empty
                    if(cur->small < word) {
                        cur->large = word;
                    }
                    else {
...
void Tree::split(Node* cur, string word, Node* leftchild, Node* rightchild) {
