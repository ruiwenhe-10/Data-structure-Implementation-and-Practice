...
void pop_front();            // Removes the node at the front end of the list 
                            //(the node after the dummy head). Does nothing if the list is already empty. This must be an O(1) operation.
...
void pop_back();             // Removes the node at the back end of the list (the node before the dummy tail). 
...
friend ostream & operator<<(ostream &out, const IntList &rhs);   //: A global friend function that outputs to the stream 
                            //all of the integer values within the list on a single line, each separated by a space. 
